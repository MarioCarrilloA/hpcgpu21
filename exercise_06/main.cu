...
// WMMA kernel
__global__ void wmma_example(half *A, half *B, half *C, float *C_wmma, int M, int N, int K, float alpha, float beta) {
    ...
    //     // Bounds checking
    //     if (cm_row < M && cm_col < N && ct_row < N && ct_col < M) {
    //         // Load the inputs
    //         //int aaa = 0;
    //         wmma::load_matrix_sync(cm_frag, C + cm_row + cm_col * ldc, ldc);
...
void matrices_computation(int m, int n, int k, float a, float b) {
    ...
    convertFp32ToFp16 <<< (k * n + 255) / 256, 256 >>> (C_fp16, C_wmma, m * n);
