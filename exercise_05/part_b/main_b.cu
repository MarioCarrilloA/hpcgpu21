...
void Print_output_as_python(float *C_dev, int n, int m) {
    ...
    cout << "print('Tensor Core:________________________')" << endl;
    cout << "print(R)" << endl;
...
__global__ void wmma_computation(half *A, half *B, float *C, int Cm, int Cn, int Ck, float alpha, float beta) {
    ...
    for(int i = 0; i < k; i += WMMA_K){
        int rowB = warpM * WMMA_M;
        int colB = i;
        int rowA = i;
        int colA = warpN * WMMA_N;

        if(rowA < k && colA < n && rowB < m && colB < k){
            // Load the inputs
...
void matrices_computation(int m, int n, int k, float alpha, float beta) {
    ...
    //Print_input_as_python(A_f32, B_f32, C_f32, m, n, k, alpha, beta);
    ...
    //Print_output_as_python(C_f32, n, m);
